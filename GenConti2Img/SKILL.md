...
> ⚠️ **`## LOCATION MAP` 절을 반드시 포함한다.** 인물도 동작도 없이 장소만 —
> 카메라 위치·바라보는 방향 / 전경·중경·배경 / 주요 랜드마크 위치 / 동선 / 조명 방향 /
> 카메라가 넘지 않는 선. (Cinedance *Location map*)
>
> **여기서 안 쓰면 영원히 안 써진다.** 파이프라인 순서가 GenConti2Img → GenVideo인데
> GenVideo는 "내용이 있으면 덮어쓰지 않음"이라 이 파일을 다시 채우지 않는다.
> 그러면 영상 단계에서 LOCATION MAP이 빈 채로 나가고 **컷이 바뀔 때마다 인물이 순간이동한다.**
> `{장소}_Location_top_v*.png`(탑다운 평면도)가 있으면 같이 읽어 근거로 삼는다.
...
### 나쁜 연기 아틀라스 — 스틸에서 자주 나오는 것 (ACTING 원문 인라인)

표정을 쓰다가 아래로 흐르면 그 자리에서 고친다. 원문이 없어도 이 표로 잡을 수 있다.

| 증상 | 어떻게 보이나 | 프롬프트에서 고치는 법 |
|---|---|---|
| **지시(mugging)** | 얼굴이 감정을 "표시"한다 — 치켜올린 눈썹, 찡그림 | 얼굴에서 과제를 뺀다. 목적을 쓰고 **손에 할 일**을 준다 |
| **결과 미리 연기** | 첫 순간부터 장면의 결말을 연기한다 | **지금 아는 것만** 쓴다 |
| **클로즈업 과잉** | 타이트한 샷에서 표정이 분주하다 | 샷이 좁을수록 움직임을 줄인다 — **눈과 생각만** |
| **죽은 눈** | 굳은 응시, 깜빡임·사케이드 없음, 유리 같은 캐치라이트 | eye life를 실제로 쓴다 (아래) |
| **몸이 이력과 어긋남** | 깡패인데 무용수 자세 | 무게중심·템포·마모를 지정한다 |
| **자유 감정** | 빌드업도 계기도 없는 눈물·분노 | 사다리를 만든다: 억제 → 붕괴. 감정에 **대가**가 있어야 한다 |

**eye life** — 스틸에서도 눈이 죽으면 인형이 된다:
마이크로 사케이드(눈↔입↔손) / 상태에 묶인 깜빡임(긴장=억제, 안도=길고 느림, 충격=멈춤) /
살아 있는 캐치라이트(광원이 실제로 맺힘) / 안 움직이는 게 아니라 **참는** 정지.
