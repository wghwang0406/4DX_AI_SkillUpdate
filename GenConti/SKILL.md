...
컬럼은 Step 6의 분해 테이블과 **같은 7컬럼**이다. 장소·소품을 비우면
GenConti2Img가 배경·소품 레퍼런스를 고를 근거를 잃는다.
기존 프로젝트의 5컬럼 파일도 그대로 읽고 쓴다 (러너가 컬럼 수를 가정하지 않는다).
...
출력 파일명: `{EP}/Conti/{SEQ_ID}/{SEQ_ID}_{SHOT_NUM}_v{N}.png` (예: `EP01/Conti/S41/S41_0010_v1.png`)
...
```bash
curl -o "{EP}/Conti/{SEQ_ID}/{SEQ_ID}_{SHOT_NUM}_v1.png" "<URL>"
```
...
files = ["{SEQ_ID}_0010_v1.png", "{SEQ_ID}_0020_v1.png", ...]
...
canvas.save(f"{base}/{SEQ_ID}_storyboard_{START}-{END}.jpg", "JPEG", quality=92)
```
4. 저장 경로 알림: `{EP}/Conti/{SEQ_ID}/{SEQ_ID}_storyboard_{START}-{END}.jpg`
