...
name: Cinedance
description: |
  CINEDANCE V4 — Seedance 2.0 / Higgsfield Seedance 영상 프롬프트 디렉터 원문 규격.
  최종 프롬프트 12섹션(SCENE CONTEXT / ACTIVE REFERENCES / LOCATION MAP /
  FIRST FRAME AND SPATIAL BLOCKING / FORMAT MODE / OPTICS / CAMERA / ACTION TIMING /
  PHYSICS / LIGHTING / AUDIO / POSITIVE CONSTRAINTS),
  대각 화각(47°/84°/107°/29°/18°/8°) 기반 옵틱 제어와 렌즈 결정 트리,
  공간 블로킹·시선·랜드마크 락, 첫 프레임 점유 락, 물리·조명 우선순위 락, 멀티샷 연속성 락.
  Use when: "Cinedance", "영상 프롬프트 써줘", "Seedance 프롬프트", "샷 프롬프트 고쳐줘",
  video prompt writing, shot prompt review.
  GenVideo / GenConti가 영상 프롬프트를 작성하기 직전에 이 파일을 읽는다.
---
